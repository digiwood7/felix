# PET Time

FDG PET 검사 준비 안내 웹서비스.

환자가 병원 도착 전에 자신의 검사 준비 상태를 스스로 정리해 오게 만들어,
핵의학과 접수 단계의 반복 문답을 줄이는 것이 목적이다.
**안내문 디지털화가 아니라 반복업무 감소 솔루션이다.**

| 문서 | 내용 |
|---|---|
| `PRD.md` | 무엇을 만들지 — 기능 명세, 룰셋, 계산 규칙, KPI |
| `WORKFLOW.md` | 어떤 순서로 만들지 — 작업 목록(T0~T13), 게이트, 측정 계획 |
| `docs/staff-review.html` | W1 게이트에서 직원에게 보여주는 검토 문서 — 배지 기준 설명 + 확정할 질문 |
| `docs/red-team.md` | 어디서 깨지는지 — 병목 · 미흡 · 환자가 이해 못할 것. 열린 항목과 담당이 표로 있다 |
| `HANDOFF.md` | 외부 협업자용 — 포크·PR 로만 들어오는 경로, 만져도 되는 파일, 디자인이 깨뜨리기 쉬운 원칙 6가지 |

세션 시작 시 두 문서를 먼저 읽는다. 한 세션 = 한 작업(T번호) = 커밋 1개.

---

## 타협 불가 원칙

1. **개인정보를 받지 않는다.**
   이름·등록번호·연락처·생년월일 입력 필드를 어떤 이유로도 추가하지 않는다.
   **자유 텍스트 입력 필드 자체를 만들지 않는다.** 문답 선택지는 룰셋에서 읽는다.
   원문이 생기는 순간 URL·공유·로그 세 곳에서 동시에 문제가 된다.

2. **의료 판단을 하지 않는다.**
   다음 표현을 생성하지 않는다:
   "검사 가능/불가", "괜찮습니다", "문제없습니다", "판단됩니다",
   "~일 수 있습니다"로 끝나는 추정 문장.
   출력은 룰셋 levels의 행동 지시 3종(ok/tell/call)뿐이다.

3. **환자에게 안내하는 시각은 모두 내림(floor) 처리한다.**
   금식 시작·복약 마지노선 → floor_hour / 도착 시각 → floor_10min.
   이르게 틀리는 방향으로만. 반올림 금지.
   **새벽 보정을 하지 않는다.** 계산값을 그대로 쓰고, 실행 조언이 필요하면
   룰셋 `fasting.note` 를 보조 한 줄로 붙인다.
   **단, 내림값을 배지 판정선으로 쓰지 않는다.** 판정은 예약 시각에서
   정확히 잰다(금식 `hours`, 당뇨약 `|offset_h|`). 내림은 **이르게
   알려주려고** 있는 것이지 **일찍 잡으려고** 있는 게 아니다 — 표시값을
   판정선으로 쓰면 최대 59분이 앞당겨져 **기준을 정확히 지킨 환자가
   걸린다.** 규칙을 지킨 환자를 접수로 보내면 그 문답이 곧 줄이려던
   업무다. 표시(`lib/schedule.ts`)와 판정(`lib/triage.ts`)은 같은 값을
   쓰지 않는다 (PRD §9.4).

4. **모든 계산은 KST 고정이다.**
   `new Date()` / `getHours()` 등 기기 로컬 타임존을 따르는 API를 계산 경로에 쓰지 않는다.
   룰셋 `timezone` 필드가 유일한 기준이다.

5. **병원명은 발신자 한 줄로만 쓴다. 로고·색상은 쓰지 않는다.**
   S1 히어로 맨 윗줄에 `삼성서울병원` 한 줄이 있다 (2026-08-28 `380b885`).
   연락처는 사실 정보로만 표기.

   **이건 원칙이 완화된 것이 아니라 빚이 하나 생긴 것이다.** 원래 이
   서비스는 병원과 무관하게 혼자 굴러갈 수 있었고, 그게 심의도 승인도
   없이 먼저 띄울 수 있었던 근거였다. 지금은 병원명을 **승인 없이**
   쓰고 있으므로 PRD §7 의 "무단 사용 시 즉시 중단 사유" 에 그대로 걸린다.

   더 급한 것은 화면 안의 모순이다. 환자는 맨 위에서 병원 이름을 보고
   맨 아래에서 "이 서비스는 병원 공식 서비스가 아니며" 를 읽는다.
   **한 화면이 스스로를 부정하고 있다.**

   → 승인 게이트에서 **둘 중 하나를 고른다.** 병원명을 빼거나,
   정식 승인을 받고 Disclaimer 문구를 함께 바꾸거나. 그때까지는
   열린 항목이다 (`docs/red-team.md` #15).

6. **모든 화면 하단에 Disclaimer를 넣는다.** (S1·S2·S3·S4 전부)
   "이 서비스는 병원 공식 서비스가 아니며, 검사 가능 여부를 판단하지 않습니다.
    최종 확인은 반드시 핵의학과 직원에게 받으세요."

---

## 아키텍처 규칙

- v1은 서버 상태를 갖지 않는다. 계산은 전부 클라이언트.
- 검사별 차이는 `lib/rules/*.json`으로만 표현한다. 코드에 검사별 분기를 넣지 않는다.
- 배지 판정은 룰셋 `flags` + `triage` 로만 한다. `lib/triage.ts` 에 조건문을 늘리지 않는다.
- 문구는 룰셋에서 읽는다. 컴포넌트에 하드코딩하지 않는다.
- 웹폰트를 추가하지 않는다. 시스템 폰트만.
- DB를 추가하지 않는다. 익명 로그만 KV.
- kill switch(`NEXT_PUBLIC_SERVICE_STATUS`)는 계산 로직보다 먼저 평가되어야 한다.

**룰셋 값이 틀렸으면 JSON을 고친다. 코드에 예외를 넣지 않는다.**
로직에 예외가 쌓이기 시작하면 검사 종류 확장이 불가능해진다.

---

## 커밋 · 푸시 · 배포는 물어보고 한다

**작업이 끝나면 화면을 보여주고 멈춘다. 커밋하지 않는다.**
`git commit` · `git push` · 배포는 **매번 따로 요청받아야** 실행한다.
"수정해줘" 는 커밋해 달라는 말이 아니다. 검사(`npm run check`)까지는
해 두고, 무엇을 바꿨는지 보고한 뒤 답을 기다린다.

**Why:** 이 프로젝트에서 화면·문구는 만들어 보고 나서 정한다. 마음에 안
들면 원안으로 되돌리는데, 이미 올라가 있으면 되돌리는 것도 커밋이 된다.
`petct.kr` 은 열려 있는 서비스라 푸시가 곧 배포다 — **환자가 보는 화면이
시안 단계에서 바뀐다.** 2026-08-28 에 이 일이 네 번 연속으로 일어났다
(`43102cb` `ff664a2` `bf17b44` `14355aa`).

되돌릴 때 쓴다: `git checkout -- <파일>`

---

## 커밋 전 확인

```bash
# 타입 · 테스트 · TZ 3종 · 금지 표현을 한 번에
npm run check

# 자유 텍스트 입력 — 0건이어야 함
grep -rn "<input type=\"text\"\|<textarea" app/ components/
```

금지 표현 검사는 `npm run lint:words` 다. `lib/disclaimer.ts` 만 제외한다 —
§11 필수 문구("검사 가능 여부를 판단하지 않습니다")가 거기 한 벌만 있다.
맨손 grep 을 돌리면 그 파일이 잡히는데, 그건 정상이다.

커밋 메시지: `feat:` `fix:` `docs:` `chore:` `test:`

**룰셋 JSON을 바꾼 커밋은 본문에 근거를 남긴다** — 어느 안내지 몇 년 몇 월판인지.
나중에 "이 값이 왜 6시간이지?"를 추적할 수 있어야 한다.

<!-- BEGIN:nextjs-agent-rules -->

# This is NOT the Next.js you know

This version has breaking changes — APIs, conventions, and file structure may all differ from your training data. Read the relevant guide in `node_modules/next/dist/docs/` (resolved from this file's directory; in monorepos the `next` package may not be visible from the repo root) before writing any code. Heed deprecation notices.

This block is written and re-added by `next dev` — verify at `node_modules/next/dist/server/lib/generate-agent-files.js`. Removing it from a diff only re-creates the uncommitted change; committing it with your work keeps the tree clean.

<!-- END:nextjs-agent-rules -->
