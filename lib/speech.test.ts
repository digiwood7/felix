...
/**
 * 화면에서는 대기실 주의사항이 접혀 있다. 소리에서는 접지 않는다.
 *
 * 듣기를 쓰는 사람은 애초에 읽기 어려운 사람이라 접힌 것을 열지 못한다.
 * 소리에는 훑기가 없으므로 감출 이유도 없다.
 */
describe("읽어주기 — 검사실 구간", () => {
  const script = scriptOf();

  it("구간 목록의 제목을 먼저 말한다", () => {
    expect(script).toContain("검사실에서는 이렇게 진행됩니다");
  });

  it("구간마다 길이와 하는 일을 한 문장으로 읽는다", () => {
    expect(script).toContain("약 50분 동안 대기실에 누워 쉽니다");
    expect(script).toContain("약 20분 동안 촬영합니다");
  });

  /**
   * 화면에서는 구간이 위아래로 놓여 화장실이 어느 시점인지 자리로 보인다.
   * 소리에는 그 배치가 없어서, 바로 앞 대기 50분에 붙은 것처럼 들린다.
   */
  it("구간마다 다른 낭독 문구를 쓸 수 있다 — 화장실", () => {
    expect(script).toContain("약 5분 촬영 직전 화장실에 다녀옵니다");
    expect(script).not.toContain("약 5분 동안 화장실에 다녀옵니다");
  });

  it("화면 문구는 그대로다 — 바뀌는 것은 소리뿐이다", () => {
    const toilet = f18FdgPet.exam.phases!.find((p) => p.id === "toilet")!;
    expect(toilet.text).toBe("화장실에 다녀옵니다");
  });

  it("접혀 있는 대기실 주의사항까지 읽는다", () => {
    expect(script).toContain("걷거나 움직이지 마시고 편히 누워 계세요");
    expect(script).toContain("보호자는 1명까지 대기실에 함께 계실 수 있습니다");
    expect(script).toContain("화장실은 중간에 가셔도 됩니다");
  });

  /**
   * 화면에서는 "대기실 주의사항" 이 누르는 줄의 이름이라 그것으로 족하다.
   * 소리에는 누를 것이 없어서, 이름만 읽고 끊기면 뒤따라오는 문장이
   * 그 주의사항인지 다른 항목인지 알 수 없다.
   */
  it("접힌 줄의 이름을 문장으로 열어 준다", () => {
    expect(script).toContain("대기실 주의사항을 알려드리겠습니다");
    // 이름만 읽고 끊기는 자리가 없다
    expect(script).not.toContain("대기실 주의사항.");
  });

  it("구간은 검사 항목 뒤에 온다 — 화면과 같은 순서다", () => {
    expect(script.indexOf("검사 시작")).toBeLessThan(
      script.indexOf("검사실에서는 이렇게 진행됩니다"),
    );
  });
});
